...
ozone_chart$xAxis(title="Temperature (F)", tickInterval=10)
ozone_chart$yAxis(title="Ozone (ppb)", gridLineColor="#FFFFFF", floor=0)
...
twitter_chart$yAxis(title="Impressions", gridLineColor="#FFFFFF")
...
x and y axes, repectively as in Fig.  `r fn()`. Just by looking at the plot, we
...
```{r, html.cap=fn("Ozone level, temperature, and wind speed")}
...
low when the ozone level high.

After we grasp the overview of the entire data, we may want to look into the
details of particular part of the data. What if we want to know the date of the
observation of the highest point of ozone level? The graph on a paper is not
ideal place to make this transition happen. Fortunately, we use computer screens
to view the graph more often these days. You can hover the mouse cursor over
Fig. `r fn()` to reveal the date of the observation.

```{r, html.cap=fn("Interactive chart of ozone level, temperature, and wind speed")}
...
within a single textfile. Having to hop around different programs to do those
acitivities is very disruptive to the thought process. With R markdown, we
...
```{r,  html.cap=fn("Interactive chart: Daigo's twitter impressions and engagements in 2014")}
...
<script src="http://gist-it.appspot.com/https://github.com/daigotanaka/essays/blob/master/publishing-interactive-data-documents.Rmd?slice=85:92&footer=0">
</script>
...
With help of packages such as rCharts, it is now much easier to publish data
...
With properly presented interactive charts, the online publishers may be able
to communicate interpretation of the data effectively.
...
footnote_contents = c(
    "New York Air Quality Measurements. For health effects of ozone, see http://www.epa.gov/groundlevelozone/health.html",
    "For example, see [Across U.S. Companies, Tax Rates Vary Greatly](http://www.nytimes.com/interactive/2013/05/25/sunday-review/corporate-taxes.html)"
    )
